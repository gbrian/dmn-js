  ...
  eventBus.on('cell.render', function(event) {
    ...
    if (row.type === 'dmn:DecisionRule' && !row.isHead && businessObject) {
      ...
      if (isType(businessObject, 'string')) {
        ...
        if (this.simple) {
          textContent = gfx.childNodes[0].textContent;

          if (this.isString(textContent)) {
            gfx.childNodes[0].textContent = textContent.replace(/^"|"$/g, '');

            var inputValues =
                data.column.businessObject.inputValues && data.column.businessObject.inputValues.text ||
                data.column.businessObject.outputValues && data.column.businessObject.outputValues.text;
            ...
            if (inputValues) {
              ...
              gfx.childNodes[0].style.display = 'none';
              if (checkbox) {
                // remove all options to redraw them
                checkbox.innerHTML = '';
                checkbox.style.display = 'inline';
              }
              newCheckbox = checkbox || domify('<select class="simple-mode-checkbox"></select>');

              var values = inputValues.split(',');
              values.forEach(function(value) {
                newCheckbox.appendChild(domify('<option value=' + value + '>' + value.substr(1, value.length - 2) + '</option>'));
              });
              newCheckbox.appendChild(domify('<option value="">-</option>'));

              // we set it readonly. An optional modeling module can make it editable
              newCheckbox.setAttribute('disabled', 'disabled');

              if (data.content && typeof data.content.text !== 'undefined') {
                var idx = data.content.text === '' ? values.length : values.indexOf(data.content.text);
                newCheckbox.selectedIndex = idx;
                if (idx === -1) {
                  hasValidValue = false;
                }
              }
              ...
              if (!hasValidValue) {
                hint = domify('<span class="expression-hint">[expression]</span>');
            ...
            hint = this.getExpressionNode(data.content);
      ...
      if (checkbox) {
        // IF NOT (
        // type is boolean OR
        // type is string AND has allowedValues
        ...
        if (!(
         ...
         businessObject.typeRef === 'string') &&
          (data.column.businessObject.inputValues &&
           data.column.businessObject.inputValues.text ||
           data.column.businessObject.outputValues &&
           data.column.businessObject.outputValues.text)
        )) {
